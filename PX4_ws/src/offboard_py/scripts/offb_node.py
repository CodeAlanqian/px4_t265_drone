...
openmv_port = serial.Serial(                #openmv串口
        port="/dev/ttyUSB0",
        baudrate=115200,
        bytesize=serial.EIGHTBITS,
        parity=serial.PARITY_NONE,
...
class serialthread(threading.Thread):
  ...
  def run(self):
        ...
        global statement
        global serial_port
        global vision_x,vision_y,openmv_flag,data1,openmv_turn_flag,stop
        rate = rospy.Rate(30)
        time.sleep(1)
        try:
            while True:
                ...
                #openmv read data
                openmv_flag = 0
                while openmv_port.inWaiting() > 0:
                    openmv_flag = 1
                    data = openmv_port.read()
                    if data != ",":
                        data1 = data1+data
                    else:    
                        if len(data1) > 0:      
                            if openmv_turn_flag:
                                    vision_x = float(data1)
                            else:
                                    vision_y = float(data1)
                            data1 = ""  
                            openmv_turn_flag = not openmv_turn_flag
                        else:
                            openmv_turn_flag = not openmv_turn_flag
...
if __name__ == "__main__":
    ...
    while(not rospy.is_shutdown() or (PX4_CTRL.current_local_pos.pose.position.x == 0.0 and PX4_CTRL.current_local_pos.pose.position.y == 0.0 and PX4_CTRL.current_local_pos.pose.position.z == 0.0)):
        if(PX4_CTRL.current_state.mode != "OFFBOARD" and (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
            if(PX4_CTRL.set_mode_client.call(PX4_CTRL.offb_set_mode).mode_sent == True):
                rospy.loginfo("OFFBOARD enabled")
                print("send_off_mode")
            last_req = rospy.Time.now()
        else:
            if(not PX4_CTRL.current_state.armed and (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
                if(PX4_CTRL.arming_client.call(PX4_CTRL.arm_cmd).success == True):
                    rospy.loginfo("Vehicle armed")
                last_req = rospy.Time.now()
                break
        PX4_CTRL.local_pub.publish(PX4_CTRL.pose)
        #print("offboard和arm状态")
        rate.sleep()
    ...
    while(not rospy.is_shutdown()):
        if statement == 0:
            if(not PX4_CTRL.current_state.armed):
                if(PX4_CTRL.arming_client.call(PX4_CTRL.arm_cmd).success == True):
                    rospy.loginfo("Vehicle armed")
            PX4_CTRL.local_pub.publish(PX4_CTRL.pose)
            if PX4_CTRL.distance_jugdge(0,0):
                statement = 1

        elif statement == 1:
            for point in setpoint_array:
                print("目标点",point[0],point[1])
                while not PX4_CTRL.distance_jugdge(point[0],point[1]) and statement == 1:
                    PX4_CTRL.sett_PID_velocity(point[0],point[1],1.2,0)
                    PX4_CTRL.vel_limit(0.5)
            ...
            while not PX4_CTRL.distance_jugdge(0,0,0):
                PX4_CTRL.sett_PID_velocity(0,0,0,0)
                PX4_CTRL.local_vel_pub.publish(PX4_CTRL.vel)
            statement = 3
            ...
            PX4_CTRL.set_vision_vel(vision_x,vision_y,1.2,0)
            PX4_CTRL.vel_limit(0.5)
            PX4_CTRL.local_vel_pub.publish(PX4_CTRL.vel)



        # print(PX4_CTRL.current_local_pos.pose.position.x,
        #          PX4_CTRL.current_local_pos.pose.position.y,
        #          PX4_CTRL.current_local_pos.pose.position.z)
        print("statement",statement)
        print("12,",vision_x,vision_y)
        # print("stop",stop)
        rate.sleep()
